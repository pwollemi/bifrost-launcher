...
describe("Bifrost", function () {
    ...
    describe("BifrostSale", function () {
        ...
        it("deposit: all go to raised after start", async () => {
            ...
            await sale.connect(bob).deposit(ethers.utils.parseEther("2"), {
                value: ethers.utils.parseEther("2")
            });

            expect(await sale.raised()).to.be.equal(raisedBefore.add(ethers.utils.parseEther("3")));
            expect(await ethers.provider.getBalance(router.address)).to.be.equal(routerBalance);
            expect(await sale._deposited(alice.address)).to.be.equal(ethers.utils.parseEther("1"));
        ...
        it("sale state", async () => {
            ...
            await sale.connect(bob).deposit(soft.div(2), { value: soft.div(2) });
            expect(await sale.successful()).to.be.equal(false);

            // successful when over soft cap
            await sale.connect(tom).deposit(soft.div(2), { value: soft });
        ...
        it("finalize and launch", async () => {
            ...
            const liquidityAmount = raised.mul(listingRate).mul(liquidity).div(1e4).div(ethers.utils.parseUnits("1", 19)).sub(devFeeTokens);
    ...
    describe("Withdraw liquidity", async () => {
        ...
        it("can only withdraw after unlock time count", async () => {
            ...
            const pair = await ethers.getContractAt("contracts/interface/uniswap/IUniswapV2Pair.sol:IUniswapV2Pair", await factory.getPair(rainbowToken.address, await prouter.WETH()))
