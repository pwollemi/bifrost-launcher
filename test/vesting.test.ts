...
import { setNextBlockTimestamp, getLatestBlockTimestamp, mineBlock, latest } from "../scripts/utils";
...
describe('Vesting', () => {
  ...
  describe("updateRecipient", async () => {
    ...
    it("Cannot update the recipient after started", async () => {
      const startTime = (await latest()).toNumber() + 10000;
  ...
  describe("setStartTime", async () => {
    ...
    it("Time is set/event emitted", async () => {
      const startTime = (await latest()).toNumber() + 100;
  ...
  describe("set/remove Worker", () => {
    it("setWorker", async () => {
      await expect(vesting.connect(signers[2]).setWorker(signers[2].address)).to.be.revertedWith("Ownable: caller is not the owner");
      await expect(vesting.connect(signers[3]).setWorker(signers[2].address)).to.be.revertedWith("Ownable: caller is not the owner");
      await expect(vesting.connect(signers[4]).setWorker(signers[2].address)).to.be.revertedWith("Ownable: caller is not the owner");
    ...
    it("removeWorker", async () => {
      await expect(vesting.connect(signers[2]).removeWorker()).to.be.revertedWith("Ownable: caller is not the owner");
      await expect(vesting.connect(signers[3]).removeWorker()).to.be.revertedWith("Ownable: caller is not the owner");
      await expect(vesting.connect(signers[4]).removeWorker()).to.be.revertedWith("Ownable: caller is not the owner");
